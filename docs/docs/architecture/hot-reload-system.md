...
To keep the client lightweight and maintainable, these core engine files are provided by the `hot-reload-runtime` library:

- **`IncrementalChange.java`**: The interface that all instrumented classes use to dispatch method calls to updated code.
- **`HotReload`**: The manager (found in `com.jetstart.hotreload`) that handles the WebSocket connection, DEX loading, and state management.
